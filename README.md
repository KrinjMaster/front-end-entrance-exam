...
## Юзер гайд к использованию сайта и установке (маст хэв прочитать)

Для того, чтобы посетить сайт нажмите на [ссылку](https://krinj-resume.vercel.app/), которая перекинет вас на сайт. Для того, чтобы изменить текстовое поле просто нажмите на него и введите нужный вам текст. Для того, чтобы скачать шаблон нажмите на кнопку `Save` и у вас начнется скачиваться резюме. Разрешение шаблона может быть плоховатым, но иначе размер файла слишком большой. 

Для того, чтобы локально запустить сайт склонируйте этот репозиторий и введите команду в терминал:
```cli
npm i && npm run dev
```
